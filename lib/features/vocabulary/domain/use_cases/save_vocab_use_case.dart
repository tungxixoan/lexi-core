...
class SaveVocabUseCase {
  ...
  Future<void> execute(VocabRecord record) async {
    final normalized =
        record.copyWith(headword: capitalizeHeadword(record.headword));
    if (normalized.inputType == InputType.sentence) {
      throw const VocabException(
          'Sentences cannot be saved to Vocabulary Bank.');
    }
    if (normalized.topicIds.length > 2) {
      throw const VocabException('A word can have at most 2 topic tags.');
    }
    final exists = await _repo.existsByHeadword(
        normalized.headword, normalized.targetLanguage);
    if (exists) {
      throw VocabException(
          '"${normalized.headword}" is already in your Vocabulary Bank.');
    }
