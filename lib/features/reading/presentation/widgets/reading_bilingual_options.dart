...
class ReadingBilingualOptions extends ConsumerStatefulWidget {
  const ReadingBilingualOptions({super.key});

  @override
  ConsumerState<ReadingBilingualOptions> createState() =>
      _ReadingBilingualOptionsState();
}

class _ReadingBilingualOptionsState
    extends ConsumerState<ReadingBilingualOptions> {
  ...
  Future<void> _reuse(BuildContext context, WidgetRef ref) async {
    ...
    final passageJson = <String, dynamic>{
      ...
      'level':
          result.generationFilters['maxCefr'] ?? result.passageJson['level'],
