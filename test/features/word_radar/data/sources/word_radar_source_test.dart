...
void main() {
  test('parses suggestions and translation from the AI JSON response',
      () async {
    ...
    expect(
        result.translation, 'Điện thoại thông minh có mặt khắp nơi hiện nay.');
    ...
    expect(result.suggestions[0].definition,
  ...
  test('returns an empty suggestions list when the AI has nothing to suggest',
      () async {
    final source = WordRadarSource.withModel(
      FakeGenerativeModelClient(
          '{"translation":"Văn bản ngắn.","suggestions":[]}'),
    );
  ...
  test('defaults translation to an empty string when the AI omits it',
      () async {
    final source = WordRadarSource.withModel(
  ...
  test('includes the known-headwords exclusion list in the prompt', () async {
    final client =
        FakeGenerativeModelClient('{"translation":"","suggestions":[]}');
  ...
  test(
      'omits the translation instruction and field from the prompt when includeTranslation is false',
      () async {
  ...
  test(
      'skips a malformed suggestion but keeps the valid ones in the same response',
      () async {
    final json = jsonEncode({
      'translation': '',
      'suggestions': [
  ...
  test(
      'defaults cefrLevel to null and keeps the suggestion when the AI returns an invalid level string',
      () async {
    final json = jsonEncode({
      'translation': '',
      'suggestions': [
  ...
  test(
      'treats a bare string as a single-item list when the AI returns one instead of a JSON array',
      () async {
    final json = jsonEncode({
      'translation': '',
      'suggestions': [
    ...
    expect(result.suggestions, hasLength(1));
    expect(result.suggestions[0].examples,
