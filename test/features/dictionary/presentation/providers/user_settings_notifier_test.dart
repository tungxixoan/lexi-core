...
void main() {
  ...
  group('UserSettingsNotifier', () {
    ...
    test('build() loads persisted provider and config from SharedPreferences',
        () async {
      final container = await makeContainer(initialValues: {
        'target_language': 'chinese',
        'ai_active_provider': 'groq',
        'ai_config_groq': jsonEncode({
          'apiKeyCiphertext': 'cipher_gsk_test',
    ...
    group('migration from old gemini_api_key', () {
      test(
          'migrates old gemini_api_key to ai_config_gemini, still in the plaintext shape',
          () async {
        final container = await makeContainer(initialValues: {
        ...
        // on the next sign-in.
        expect(
            notifier.legacyPlaintextApiKey(AiProvider.gemini), 'old-key-xyz');
        ...
        final geminiJson = jsonDecode(prefs.getString('ai_config_gemini')!)
    ...
    test('setActiveProvider() updates state and writes to prefs', () async {
      ...
      container
          ...
          .setActiveProvider(AiProvider.groq);
    ...
    test('setProviderConfig() persists JSON and updates state', () async {
      ...
      const config = ProviderConfig(
          apiKeyCiphertext: 'cipher-my-key', model: 'llama-3.1-8b-instant');
      container
          .read(userSettingsNotifierProvider.notifier)
      ...
      expect(state.providerConfigs[AiProvider.groq]?.apiKeyCiphertext,
          'cipher-my-key');
      expect(state.providerConfigs[AiProvider.groq]?.model,
          'llama-3.1-8b-instant');
      final stored = jsonDecode(prefs.getString('ai_config_groq')!)
          as Map<String, dynamic>;
    ...
    test(
        'setApiKeyCiphertextForActiveProvider() keeps model, updates ciphertext',
        () async {
      final container = await makeContainer(initialValues: {
        'ai_active_provider': 'gemini',
        'ai_config_gemini': jsonEncode(
            {'apiKeyCiphertext': 'old-cipher', 'model': 'gemini-2.5-pro'}),
      });
      addTearDown(container.dispose);
      container
          .read(userSettingsNotifierProvider.notifier)
    ...
    test('setModelForActiveProvider() keeps ciphertext, updates model',
        () async {
      final container = await makeContainer(initialValues: {
        'ai_active_provider': 'gemini',
        'ai_config_gemini': jsonEncode(
            {'apiKeyCiphertext': 'test-cipher', 'model': 'gemini-2.5-flash'}),
      });
      addTearDown(container.dispose);
      container
          .read(userSettingsNotifierProvider.notifier)
    ...
    test('setTargetCefrLevel(null) removes key from prefs', () async {
      ...
      expect(
          container.read(userSettingsNotifierProvider).targetCefrLevel, isNull);
    ...
    group('reminder settings', () {
      test('reminderEnabled defaults to false', () async {
        final container = await makeContainer();
        addTearDown(container.dispose);
        expect(container.read(userSettingsNotifierProvider).reminderEnabled,
            false);
      });
      ...
      test('setReminderEnabled persists to prefs and updates state', () async {
        ...
        expect(
            container.read(userSettingsNotifierProvider).reminderEnabled, true);
      });

      test('reminderHour defaults to 20 and setReminderHour persists',
          () async {
        final container = await makeContainer();
        addTearDown(container.dispose);
    ...
    group('Firestore push on change', () {
      ...
      test(
          'setProviderConfig(sync: false) does not push (used by bootstrap merge)',
          () async {
        final fake = _FakeAiSettingsSyncService();
        final container = await makeContainer(extraOverrides: [
          currentUidProvider.overrideWithValue('uid-1'),
        ...
        container.read(userSettingsNotifierProvider.notifier).setProviderConfig(
            ...
            const ProviderConfig(
                apiKeyCiphertext: 'c', model: 'gemini-2.5-flash'),
            sync: false);
        await Future<void>.delayed(Duration.zero);
        expect(fake.pushCount, 0);
      });
    ...
    group('legacyPlaintextApiKey', () {
      test('returns the raw key from a pre-migration JSON shape', () async {
        final container = await makeContainer(initialValues: {
          'ai_config_gemini': jsonEncode(
              {'apiKey': 'old-plain-key', 'model': 'gemini-2.5-flash'}),
        });
        addTearDown(container.dispose);
        final notifier = container.read(userSettingsNotifierProvider.notifier);
        expect(
            notifier.legacyPlaintextApiKey(AiProvider.gemini), 'old-plain-key');
      });

      test('returns null once migrated to the ciphertext shape', () async {
        final container = await makeContainer(initialValues: {
          'ai_config_gemini': jsonEncode(
              {'apiKeyCiphertext': 'cipher', 'model': 'gemini-2.5-flash'}),
        });
        addTearDown(container.dispose);
        final notifier = container.read(userSettingsNotifierProvider.notifier);
