# Plan 5 — Task 05: ProgressScreen UI + Route + AppBar Icon

**Context:** Task 05 of Plan 5. Tasks 01–04 must be complete. See `plan5-global-constraints.md` for project-wide rules.
...
**Interfaces:**
- Consumes:
  - `learningStatsProvider` → `LearningStats` (from Task 04)
  - `getVocabListUseCaseProvider` (existing) with `dueOnly: true` (from Task 03)
  - `SessionConfig` (existing entity from Practice feature)
- Produces: `ProgressScreen` widget at route `/practice/progress`

**UI sections in ProgressScreen:**
1. Streak banner (🔥 if streak > 0, ❄️ if 0)
2. Two stat cards: "Hôm nay" (dueCount) + "Đã thuộc" (masteredCount)
3. "Ôn N từ ngay" FilledButton — only shown if dueCount > 0
4. 7-day bar chart (weekly log)
5. CEFR breakdown (LinearProgressIndicator per level)

**AppBar:** PracticeHomeScreen gets a `Icons.bar_chart_outlined` icon in `actions` that pushes to `/practice/progress`.
...
- [ ] **Step 2: Add /practice/progress route**
...
Inside the `/practice` `GoRoute`'s `routes` list, add `progress` as the **first** child (before `session`):
```dart
GoRoute(
  path: 'progress',
...
- [ ] **Step 3: Add 📊 AppBar icon to PracticeHomeScreen**

In `lib/features/practice/presentation/screens/practice_home_screen.dart`, find the `Scaffold` and update its `appBar`:
...
```
git add lib/features/practice/presentation/screens/progress_screen.dart \
