...
A stale prefetch from a discarded session (e.g. "Câu khác" fires before the previous prefetch resolved) needs no new handling: the existing sessionKey-guarded reset effect already clears `fullClipUrlRef` and (with this change) `prefetchPromiseRef` on every genuine session change, so a late-resolving stale promise is simply never read by the new session's `play()`.
