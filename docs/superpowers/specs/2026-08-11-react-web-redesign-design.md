...
| LLM prompt calls | Move server-side, BYOK key never stored in DB **(superseded — see §3.5 REVISED)** | Hides the key from third-party domains and centralizes validation/streaming; storing keys server-side would create a single high-value breach target disproportionate to this app's scale |
...
- **Per-language `vocab_records` collections** (2026-08-16, user-requested, decision recorded but NOT implemented): now that Cài đặt has a "Ngôn ngữ mục tiêu" (target language) setting, the user wants `users/{uid}/vocab_records` split into one Firestore subcollection per target language — e.g. `users/{uid}/vocab_records_english`, `users/{uid}/vocab_records_chinese`, etc. (naming pattern proposed, not yet finalized) — instead of the current single flat collection filtered client-side by each record's `targetLanguage` field. **Explicitly scoped to `vocab_records` only** — `topics` stays a single shared collection, not split per language. **Not touched in this pass**: this is a real, nontrivial migration — it would change `apps/web/src/lib/vocabRecords.ts`'s `getVocabRecords(uid)` signature (needs a `language` parameter), requires a one-time data-migration script for the 290+ real production records (routing each existing record into its new per-language collection based on its own `targetLanguage` field), and requires a matching change on the Flutter side (`lib/features/vocabulary/data/repositories/vocab_repository_impl.dart`) since both apps read/write the same Firestore data — Flutter is explicitly out of scope for this decision the same way it's deferred for the streak collection and the BYOK-key Firestore migration elsewhere in this doc. Needs its own spec → plan cycle before implementation; this entry exists so the decision isn't lost in the meantime.
