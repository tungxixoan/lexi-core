...
- On each AI-backed call, the key travels as a field in the `httpsCallable` **`data` payload** (never a URL query string, which would leak into logs). The Cloud Function uses it in-memory for that one upstream call and never logs or caches it.
